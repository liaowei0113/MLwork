...
def ExtractNeededData(i, MergedData_Dict):
# 某一天的所需数据提取
    OneDayData = []
    OneDayData.append(MergedData_Dict[i]['AQI'])
    OneDayData.append(MergedData_Dict[i]['PM2.5'])
    OneDayData.append(MergedData_Dict[i]['PM10'])
    OneDayData.append(MergedData_Dict[i]['SO2'])
    OneDayData.append(MergedData_Dict[i]['CO'])
    OneDayData.append(MergedData_Dict[i]['NO2'])
    OneDayData.append(MergedData_Dict[i]['O3_8h'])

    DayNight = MergedData_Dict[i]['天气状况'].split('/') # 将白天晚上的天气状况数据分开
...
def EncodeData(MergedData):
    EncodedList = []
    MergedData_Dict = MergedData.to_dict(orient = 'record') # dataframe转化为dict数据
    # [{'日期': '2013-12-02', 'AQI': 142, '质量等级': '轻度污染', 
    # 'PM2.5': 109, 'PM10': 138, 'SO2': 61, 'CO': 2.6, 'NO2': 88, 
    # 'O3_8h': 11, '天气状况': '多云/多云', '气温': '11℃/-1℃', 
    # '风力风向': '无持续风向≤3级/无持续风向≤3级'}, .... ,{}]
    DayLength = len(MergedData_Dict)  # 一共多少天的数据
    for i in range(0, DayLength):
        if i >= 2:
            TheDayData = ExtractNeededData(i, MergedData_Dict) # 当天数据
            Last1DayData = ExtractNeededData(i-1, MergedData_Dict) # 前一天数据
            Last2DayData = ExtractNeededData(i-2, MergedData_Dict) # 前二天数据
            PredictOneDayData = TheDayData + Last1DayData + Last2DayData
                        #  预测某一天等级的全部数据，包括当天及前两天的空气数据和天气状况
            QuaLevFlag = EncodeAirQualityLevel(MergedData_Dict[i]['质量等级'])
                        # 分类编码，共6种类别
...
if __name__ == '__main__':
    BJMergedData = pd.read_excel(r'重庆合并数据.xlsx')  # 读取数据,得到DataFrame
