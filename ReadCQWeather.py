...
def ExtractCQWeather():
    # 爬取重庆天气数据
    CQWeatherExcel = xlsxwriter.Workbook('重庆天气爬虫.xlsx')
        # 创建Excel表格重庆天气爬虫.xlsx,之后每年添加一个sheet
    url = 'http://www.tianqihoubao.com/lishi/chongqing.html'
    ...
    for AQuarter in OneYearWeather:  # 一个季度的数据
        Year += 1
        if Year <= 2020: # 首先底部还有许多冗余信息会被爬进去，加上年份限制
            WorkSheet = CQWeatherExcel.add_worksheet(str(Year))
                # 添加一个sheet，命名为该年，worksheet指向CQWeatherExcel
            row = 0
            col = 0
